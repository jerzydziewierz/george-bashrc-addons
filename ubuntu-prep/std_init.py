...
logger=logging.getLogger('<your_region_name>')
...
_available_functions = "Timestamp() "
...
_available_functions += "nnlog(log,message) "


def nnlog(log=None, message='message unset'):
    """
    Shortcut: conditional logging: if log is set, log.info it with the message.
    :param log:
    :param message:
    :return:
    """
...
_available_functions += "make_log(section_name, verbose) "
...
# auxiliary tools
_available_functions += "run_with_shell(command) "
...
_available_functions += "px_show(command) "
...
_available_functions += "px_describe(hf, title, xaxis, yaxis, fontsize) "


def px_describe(hf=None, title='long title', xaxis='xaxis[units]', yaxis='yaxis[units]', fontsize=14):
    """
    px_describe(hf, title, xaxis, yaxis, fontsize) -
    ...
    if hf is None:
        raise Exception('please provide a plotly figure in hf=...')
    hf.layout.title.text = title
    hf.layout.xaxis.title.text = xaxis
    hf.layout.yaxis.title.text = yaxis
...
_available_functions += "px_axis_equal(hf) "
...
_available_modules += "numpy, plotly, px, go, pandas, json, tau, jax, chevron"
import numpy
...
# folder_data = Path(os.getcwd() + '/../data/raw/').resolve().__str__()
...
welcome_text += f'std_init ver 220317 in "QLM" : ready. Have a productive day!'
